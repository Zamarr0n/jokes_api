...
const getAllJokes = async(req,res) => {
    //#swagger.tags-['Jokes']
    const result = await client.db("Jokes").collection("Halloween").find()
...
const getJoke = async(req,res) => {
    ...
    const result = await client.db('Jokes').collection('Halloween').find({_id:jokeId})
    result.toArray().then((joke) => {
        res.setHeader('Content-Type', 'application/json');
...
const newJoke = async (req,res) => {
    ...
    const response = await client.db('Jokes').collection('Halloween').insertOne(joke)
...
const updateJoke = async(req,res) => {
    ...
    const response = await client.db('Jokes').collection('Halloween').replaceOne({_id: jokeId}, joke);
...
const deleteJoke = async(req,res) => {
    //#swagger.tags-['Jokes']
    const jokeId = new Objectid(req.params.id);
    const response = await client.db('Jokes').collection('Halloween').deleteOne({_id: jokeId});
