...
const getAllJokes = async(req,res) => {
    //#swagger.tags-['Jokes']
    const result = await client.db("Jokes").collection("CS_jokes").find()
...
const getJoke = async(req,res) => {
    ...
    const result = await client.db('Jokes').collection('CS_jokes').find({_id:jokeId})
    result.toArray().then((joke) => {
        res.setHeader('Content-Type', 'application/json');
...
const newJoke = async (req,res) => {
    ...
    const response = await client.db('Jokes').collection('CS_jokes').insertOne(joke)
...
const updateJoke = async(req,res) => {
    ...
    const response = await client.db('Jokes').collection('CS_jokes').replaceOne({_id: jokeId}, joke);
...
const deleteJoke = async(req,res) => {
    //#swagger.tags-['Jokes']
    const jokeId = new Objectid(req.params.id);
    const response = await client.db('Jokes').collection('CS_jokes').deleteOne({_id: jokeId});
